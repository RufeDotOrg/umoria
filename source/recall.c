...
int mon_num;
{
  ...
  if (mp->r_deaths) {
    (void)sprintf(temp, "%d of the contributors to your monster memory %s",
                  mp->r_deaths, ((mp->r_deaths) == 1 ? "has" : "have"));
    roff(temp);
  ...
  for (i = 0; *pu != 0 && i < 4; pu++, i++) {
    ...
    if (att_type != 1 || d1 > 0 && d2 > 0) {
      ...
      if (d1 && d2) {
        if (((4 + (cp->level)) * (mp->r_attacks[i]) > 80 * (d1 * d2))) {
