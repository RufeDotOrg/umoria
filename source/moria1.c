    ...
    switch (command) {
      ...
      case 'd': /* Drop */
        if (inven_ctr == 0 && equip_ctr == 0)
          ...
          selecting = TRUE;
          if ((scr_state == 1 && equip_ctr > 0) || inven_ctr == 0) {
            if (scr_state != 0) inven_screen(1);
      ...
      if (from > to)
        ...
        /* Swap screens (for drop) */
          ...
          else /* Found an item! */
          {
            ...
            if (free_turn_flag == FALSE && scr_state == 0)
              selecting = FALSE;
          }
        }
      }
    }
    if (which == ESCAPE || scr_state == 0)
      command = ESCAPE;
