      ...
      do {
        ...
        if (dir == 5) {
          for (i = 1; i <= 4; i++) {
            ...
            gl_fxy = set_fxy[i];
            gl_fyy = set_fyy[i];
            if (look_ray(0, 2 * 10000 - 1, 1)) {
              abort = TRUE;
              break;
            }
            gl_fxy = -gl_fxy;
            gl_fyy = -gl_fyy;
            if (look_ray(0, 2 * 10000, 2)) {
              abort = TRUE;
              break;
            }
          }
        } else if ((dir & 1) == 0) /* Straight directions */
        {
          ...
          gl_fxy = set_fxy[i];
          gl_fyy = set_fyy[i];
          if (look_ray(0, 10000, 1))
            abort = TRUE;
          else {
            gl_fxy = -gl_fxy;
            gl_fyy = -gl_fyy;
            abort = look_ray(0, 10000, 2);
          }
        } else {
          i = map_diag1[dir >> 1];
          gl_fxx = set_fxx[i];
          ...
          if (look_ray(1, 2 * 10000, 10000))
            ...
            gl_fxy = set_fxy[i];
            gl_fyy = set_fyy[i];
            abort = look_ray(1, 2 * 10000 - 1, 10000);
          }
        }
