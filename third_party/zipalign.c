...
static void
GetDosLocalTime(int64_t utcunixts, uint16_t* out_time, uint16_t* out_date)
{
  struct tm tm;
...
static int
NormalizeMode(int mode)
{
  int res = mode & S_IFMT;
  if (mode & 0111) res |= 0111;
  return res | 0644;
}
...
main(int argc, char* argv[])
{
  ...
  while ((opt = getopt(argc, argv, "0123456789vjNa:")) != -1) {
    switch (opt) {
      ...
      case 'a':
        ...
        if (flag_alignment < 1) Die(prog, "flag_alignment must be at least 1");
  ...
  if (optind == argc) Die(prog, "missing output argument");
  ...
  if ((zfd = open(zpath, O_CREAT | O_RDWR, 0644)) == -1) DieSys(zpath);
  if ((zsize = lseek(zfd, 0, SEEK_END)) == -1) DieSys(zpath);
  ...
  if (pread(zfd, last64, amt, off) != amt) DieSys(zpath);
  ...
  if (pread(zfd, cdir, cdirsize, off) != cdirsize) DieSys(zpath);
  ...
  for (int i = optind; i < argc; ++i) {
    ...
    if (!flag_level) {
      ...
      zs.zfree = 0;
      zs.opaque = 0;
      switch (deflateInit2(&zs, flag_level, Z_DEFLATED, -MAX_WBITS,
    ...
    for (off_t i = 0; i < size; i += rc) {
      // read chunk
      if ((rc = pread(fd, iobuf, Min(size, CHUNK), i)) <= 0) DieSys(path);
      ...
      if (!flag_level) {
        ...
        do {
          ...
          int boop;
          switch (
              (boop = deflate(&zs, rc != CHUNK ? Z_FINISH : Z_FULL_FLUSH))) {
            case Z_MEM_ERROR:
              DieOom();
            case Z_STREAM_ERROR:
    ...
    if (flag_level) npassert(deflateEnd(&zs) == Z_OK);
    ...
    if (pwrite(zfd, lochdr, hdrlen, zsize) != hdrlen) DieSys(zpath);
    free(lochdr);
    ...
    p = ZIP_WRITE32(p, NormalizeMode(st.st_mode)
                           << 16);    // external file attributes
    ...
    // log asset creation
    if (flag_verbose) tinyprint(2, path, " -> ", name, "\n", NULL);
  }

  // write out central directory
  if (pwrite(zfd, cdir, cdirsize, zsize) != cdirsize) DieSys(zpath);
  free(cdir);
